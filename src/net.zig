...
pub const Stream = struct {
    ...
    pub fn read(self: Stream, buffer: []u8) ReadError!usize {
        ...
        return neco.read(self.handle, buffer);
    }
    ...
    pub fn readv(s: Stream, iovecs: []const posix.iovec) ReadError!usize {
        ...
        if (iovecs.len == 0) return @as(usize, 0);
        const first = iovecs[0];
        return neco.read(s.handle, first.base[0..first.len]);
    }
    ...
    pub fn write(self: Stream, buffer: []const u8) WriteError!usize {
        ...
        return neco.write(self.handle, buffer);
    }
    ...
    pub fn writev(self: Stream, iovecs: []const posix.iovec_const) WriteError!usize {
        if (iovecs.len == 0) return @as(usize, 0);
        const first = iovecs[0];
        return neco.write(self.handle, first.base[0..first.len]);
    }
...
pub const Server = struct {
    ...
    pub fn accept(s: *Server) AcceptError!Connection {
        ...
        const fd = try neco.accept(s.stream.handle, &accepted_addr.any, &addr_len);
        return .{
            .stream = .{ .handle = fd },
